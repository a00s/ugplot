...
# Validação de instalação: falha o build se o pacote/extdata não estiverem disponíveis
RUN R -e "stopifnot(requireNamespace('ugplot', quietly = TRUE)); f <- system.file('extdata', '2dplotlist.csv', package = 'ugplot'); stopifnot(nzchar(f), file.exists(f))"
