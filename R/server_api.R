...
ugplot_discovery_report_html <- function(job_id = "") {
  ...
  paste0('<!doctype html><html lang="en"><head><meta charset="utf-8">',
    ...
    'const expandedRows=new Set(),trackCache=new Map();const rowKey=r=>[r.group||"",r.stratum||""].join("\\r");function metricCard(label,value){return `<div class="detail-metric"><b>${value}</b><span>${label}</span></div>`}function detailHtml(r,key){return `<tr class="detail-row"><td colspan="14"><div class="row-detail"><div class="detail-metrics">${metricCard("Median R²",fmt(r.median_r2))}${metricCard("Min R²",fmt(r.min_r2))}${metricCard("Max R²",fmt(r.max_r2))}${metricCard("Metric SE",fmt(r.metric_se))}${metricCard("Seeds",fmt(r.seeds).replace(".000",""))}${metricCard("Samples",fmt(r.samples).replace(".000",""))}${metricCard("Stratum",esc(r.stratum)||"—")}${metricCard("Resolved by",esc(r.resolved_by)||"—")}</div><div class="track-box" data-track-group="${esc(r.group)}"><div class="track-state">Loading CpG genomic positions…</div></div></div></td></tr>`}',
    ...
    'function drawTrack(host,payload){host.replaceChildren();const points=(Array.isArray(payload.points)?payload.points:Object.values(payload.points||{})).map(normalize).filter(p=>Number.isFinite(Number(p.position)));if(!points.length){const state=document.createElement("div");state.className="track-state";state.textContent="CpG genomic positions are not available for this group yet.";host.append(state);return}const txs=[...new Set(points.map(p=>String(p.transcript||"Transcript")))];const W=1120,left=125,right=45,top=35,rowH=72,bottom=38,H=top+txs.length*rowH+bottom;const min=Math.min(...points.map(p=>Number(p.position))),max=Math.max(...points.map(p=>Number(p.position)));const span=Math.max(1,max-min),x=v=>left+(Number(v)-min)/span*(W-left-right);const svg=svgNode("svg",{class:"track-svg",viewBox:`0 0 ${W} ${H}`,role:"img","aria-label":`${payload.group||"Group"} CpG genomic track`});txs.forEach((tx,ti)=>{const y=top+ti*rowH+30,rows=points.filter(p=>String(p.transcript||"Transcript")===tx).sort((a,b)=>Number(a.position)-Number(b.position));svg.append(svgNode("text",{x:8,y:y+4,fill:"#34405f","font-size":12,"font-weight":750},tx));svg.append(svgNode("line",{x1:left,x2:W-right,y1:y,y2:y,stroke:"#68778a","stroke-width":1.4}));rows.forEach((p,i)=>{const region=primaryRegion(p.region);if(region){const x0=i?x((Number(rows[i-1].position)+Number(p.position))/2):Math.max(left,x(p.position)-7),x1=i<rows.length-1?x((Number(rows[i+1].position)+Number(p.position))/2):Math.min(W-right,x(p.position)+7);svg.append(svgNode("rect",{x:x0,y:y-11,width:Math.max(2,x1-x0),height:22,fill:regionColors[region],opacity:.28}))}});rows.forEach(p=>{const px=x(p.position),rho=Number(p.rho),signal=Number.isFinite(rho)?rho:0,py=y-signal*25,abs=Number.isFinite(Number(p.abs_rho))?Number(p.abs_rho):Math.abs(signal),opacity=p.kept===false?.32:1,color=rhoColor(abs);svg.append(svgNode("line",{x1:px,x2:px,y1:y,y2:py,stroke:color,"stroke-width":2,opacity}));const circle=svgNode("circle",{cx:px,cy:py,r:4.8,fill:"white",stroke:color,"stroke-width":2.2,opacity});circle.append(svgNode("title",{},`${p.cpg||"CpG"} | ${p.chr||""}:${p.position} | ${p.region||"region unknown"} | rho ${Number.isFinite(rho)?rho.toFixed(4):"—"}`));svg.append(circle)})});svg.append(svgNode("text",{x:left,y:H-10,fill:"#7a849f","font-size":11},Math.round(min).toLocaleString()));svg.append(svgNode("text",{x:W-right,y:H-10,fill:"#7a849f","font-size":11,"text-anchor":"end"},Math.round(max).toLocaleString()));svg.append(svgNode("text",{x:(left+W-right)/2,y:H-10,fill:"#59657d","font-size":11,"text-anchor":"middle"},"Genomic position"));host.append(svg)}',
    ...
    'function render(){const q=$("search").value.toLowerCase();const mode=$("sort").value;const score=r=>{const ml=Number(r.median_r2),cpg=Math.abs(Number(r.cpg_rho));if(mode==="combined"){const values=[ml,cpg].filter(Number.isFinite);return values.length?Math.max(...values):-Infinity}const v=mode==="cpg"?cpg:ml;return Number.isFinite(v)?v:-Infinity};const data=all.filter(r=>Object.values(r).join(" ").toLowerCase().includes(q)).sort((a,b)=>columnSort?columnCompare(a,b):score(b)-score(a));$("rows").innerHTML=data.map((r,i)=>{const key=rowKey(r),open=expandedRows.has(key);const main=`<tr class="data-row" tabindex="0" data-row-index="${i}" aria-expanded="${open}"><td class="expand-cell"><span class="expand-icon">›</span></td><td>${badge(esc(r.status))}</td><td>${typeBadge(esc(r.type))}</td><td><b>${memberList(r.gene)}</b></td><td>${memberList(r.transcript)}</td><td>${esc(r.best_cpg)}</td><td>${fmt(r.cpgs).replace(".000","")}</td><td>${fmt(r.cpg_rho)}</td><td>${esc(r.model)}</td><td><b>${fmt(r.median_r2)}</b></td><td>${fmt(r.seeds).replace(".000","")}</td><td>${fmt(r.samples).replace(".000","")}</td><td>${resolverBadge(r.resolved_by)}</td><td>${esc(r.group)}</td></tr>`;return main+(open?detailHtml(r,key):"")}).join("");document.querySelectorAll(".data-row").forEach(row=>{const toggle=()=>{const item=data[Number(row.dataset.rowIndex)],key=rowKey(item);expandedRows.has(key)?expandedRows.delete(key):expandedRows.add(key);render()};row.onclick=toggle;row.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();toggle()}}});document.querySelectorAll(".track-box[data-track-group]").forEach(host=>loadTrack(host,host.dataset.trackGroup));$("empty").style.display=data.length?"none":"block"}$("search").oninput=render;$("sort").onchange=()=>{columnSort=null;updateHeaderSort();render()};',
