...
ugplot_job_discovery_report_paths <- function(job_id,
  ...
  list(
    ...
    group_details = group_paths$details,
...
ugplot_read_job_geo_group_dataset <- function(job_id, group_id,
  ...
  details <- ugplot_read_discovery_csv(paths$group_details)
  if (is.data.frame(details) && nrow(details) > 0L && "GroupID" %in% names(details)) {
    details <- details[toupper(as.character(details$GroupID)) == toupper(group_id), , drop = FALSE]
    rownames(details) <- NULL
  } else {
    details <- data.frame()
  }
  importance <- data.frame()
  for (summary_path in c(paths$stability, paths$screening)) {
    summary <- ugplot_read_discovery_csv(summary_path)
    if (!is.data.frame(summary) || nrow(summary) == 0L ||
        !all(c("GroupID", "ImportancePath") %in% names(summary))) next
    summary <- summary[toupper(as.character(summary$GroupID)) == toupper(group_id), , drop = FALSE]
    if (nrow(summary) == 0L) next
    importance_paths <- unique(as.character(summary$ImportancePath))
    importance_paths <- importance_paths[!is.na(importance_paths) & nzchar(importance_paths) & file.exists(importance_paths)]
    if (length(importance_paths) == 0L) next
    importance <- ugplot_read_discovery_csv(importance_paths[[1]])
    if (is.data.frame(importance) && nrow(importance) > 0L) break
  }
  list(
...
.ugplot_discovery_cpg_rho_cache <- new.env(parent = emptyenv())

ugplot_discovery_dataset_cpg_spearman <- function(path, cpg, target_column,
  ...
  stratum_column <- as.character(stratum_column %||% "")
  stratum_value <- as.character(stratum_value %||% "")
  if (length(path) != 1L || !nzchar(path) || !file.exists(path) ||
      length(cpg) != 1L || !nzchar(cpg)) return(empty)
  info <- file.info(path)
  signature <- paste(info$size[[1]], as.numeric(info$mtime[[1]]), cpg,
                     target_column, stratum_column, stratum_value, sep = ":")
  cache_key <- paste(normalizePath(path, winslash = "/", mustWork = FALSE),
                     cpg, target_column, stratum_column, stratum_value, sep = "\r")
  cached <- .ugplot_discovery_cpg_rho_cache[[cache_key]]
  if (is.list(cached) && identical(cached$signature, signature)) return(cached$value)
  dataset <- tryCatch(
    if (grepl("\\.rds$", path, ignore.case = TRUE)) readRDS(path) else
      utils::read.csv(path, stringsAsFactors = FALSE, check.names = FALSE),
    error = function(e) data.frame()
  )
  if (nzchar(stratum_column)) {
    if (!is.data.frame(dataset) || !stratum_column %in% names(dataset)) return(empty)
    dataset <- dataset[as.character(dataset[[stratum_column]]) == stratum_value, , drop = FALSE]
  }
  value <- ugplot_geo_cpg_spearman(dataset, cpg, target_column)
  .ugplot_discovery_cpg_rho_cache[[cache_key]] <- list(signature = signature, value = value)
  value
}
...
ugplot_job_discovery_report <- function(job_id,
  ...
  rows <- lapply(report_rows, function(row) {
    ...
    cpg_rho_original <- abs(number_value(row, c("TriggerBestRho", "TriggerMaxAbsRho")))
    cpg_rho_ml_signed <- number_value(row, "TriggerBestRhoML")
    cpg_rho_ml_n <- number_value(row, "TriggerBestNML")
    if (!is.finite(cpg_rho_ml_signed)) {
      recalculated <- ugplot_discovery_dataset_cpg_spearman(
        text_value(row, "DatasetPath"),
        text_value(row, c("TriggerBestCpG", "BestCpG")),
        as.character(paths$config$target_column %||% "target"),
        text_value(row, "StratumColumn"),
        text_value(row, "StratumValue")
      )
      cpg_rho_ml_signed <- recalculated[["rho"]]
      cpg_rho_ml_n <- recalculated[["n"]]
    }
    cpg_rho_ml <- abs(cpg_rho_ml_signed)
    cpg_rho <- if (is.finite(cpg_rho_ml)) cpg_rho_ml else cpg_rho_original
    ...
    data.frame(
      ...
      cpg_rho = cpg_rho,
      cpg_rho_original = cpg_rho_original,
      cpg_rho_ml = cpg_rho_ml,
      cpg_rho_ml_n = cpg_rho_ml_n,
...
ugplot_job_discovery_group_track <- function(job_id, group_id,
                                             jobs_dir = ugplot_default_jobs_dir()) {
  group_id <- trimws(as.character(group_id %||% ""))
  if (length(group_id) != 1L || !grepl("^TG[0-9]+$", group_id, ignore.case = TRUE)) {
    stop("Invalid transcript group id.", call. = FALSE)
  ...
  details <- ugplot_read_discovery_csv(paths$group_details)
  if (!is.data.frame(details) || nrow(details) == 0L || !"GroupID" %in% names(details)) {
    return(list(group = group_id, points = list()))
  }
  details <- details[toupper(as.character(details$GroupID)) == toupper(group_id), , drop = FALSE]
  if (nrow(details) == 0L || !all(c("CpG", "Position") %in% names(details))) {
    return(list(group = group_id, points = list()))
  }
  text <- function(row, name) {
    value <- if (name %in% names(row)) as.character(row[[name]][[1]] %||% "") else ""
    ugplot_public_report_text(value, 160L)
  }
  number <- function(row, name) {
    value <- if (name %in% names(row)) suppressWarnings(as.numeric(row[[name]][[1]])) else NA_real_
    if (length(value) == 0L || !is.finite(value)) NULL else value
  }
  points <- unname(lapply(seq_len(nrow(details)), function(i) {
    row <- details[i, , drop = FALSE]
    rho <- number(row, "SpearmanRho")
    abs_rho <- number(row, "AbsRho")
    if (is.null(abs_rho) && !is.null(rho)) abs_rho <- abs(rho)
    list(
      transcript = text(row, "Transcript"), cpg = text(row, "CpG"),
      region = text(row, "GeneRegion"), chr = text(row, "Chr"),
      position = number(row, "Position"), strand = text(row, "Strand"),
      rho = rho, abs_rho = abs_rho,
      kept = if ("CpGKeptForML" %in% names(row)) isTRUE(as.logical(row$CpGKeptForML[[1]])) else TRUE
    )
  }))
  points <- Filter(function(point) !is.null(point$position), points)
  list(group = group_id, points = points)
}
...
ugplot_discovery_report_html <- function(job_id = "") {
  ...
  paste0('<!doctype html><html lang="en"><head><meta charset="utf-8">',
    ...
    '.track-box{min-height:430px;overflow:visible;padding:18px}.track-state{min-height:390px}.track-transcripts{min-width:0;margin-bottom:12px;font-size:13px}.track-layout{display:grid;grid-template-columns:minmax(720px,1fr) 210px;gap:20px;align-items:stretch}.track-plot{min-width:0;overflow-x:auto}.track-svg{min-width:720px}.track-legend{padding:16px;border:1px solid #e2e6f2;border-radius:13px;background:#f8faff;color:#52607e;font-size:11px;line-height:1.35}.track-legend>strong{display:block;color:var(--ink);font-size:13px;margin-bottom:13px}.legend-item{display:grid;grid-template-columns:24px 1fr;gap:8px;align-items:center;margin-bottom:12px}.legend-item b{display:block;color:#34405f;font-size:11px}.legend-item small{display:block;color:var(--muted);font-size:10px}.rho-gradient{width:24px;height:8px;border-radius:999px;background:linear-gradient(90deg,hsl(270 84% 34%),hsl(137 84% 43%),hsl(4 84% 52%))}.height-mark{position:relative;width:24px;height:26px;border-bottom:1px solid #68778a}.height-mark:before{content:"";position:absolute;left:11px;bottom:0;width:2px;height:23px;background:#1ba6a1}.height-mark:after{content:"";position:absolute;left:7px;top:0;width:10px;height:10px;border:2px solid #1ba6a1;border-radius:50%;background:#fff}.faded-mark{width:13px;height:13px;margin-left:5px;border:2px solid #5d47c5;border-radius:50%;opacity:.32}.legend-regions{padding-top:10px;border-top:1px solid #e2e6f2}.legend-regions>b{display:block;color:#34405f;margin-bottom:8px}.region-key{display:flex;align-items:center;gap:7px;margin:6px 0}.region-key i{width:13px;height:13px;border-radius:3px}.region-promoter{background:#f6c85f}.region-utr{background:#8ecae6}.region-exon{background:#4dab6d}.region-intron{background:#d9e2ec}.region-body{background:#b8c0ff}.region-utr3{background:#c77dff}@media(max-width:1050px){.track-layout{grid-template-columns:1fr}.track-legend{display:grid;grid-template-columns:repeat(3,minmax(150px,1fr));gap:10px}.legend-regions{grid-column:1/-1;display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding-top:10px}.legend-regions>b{margin:0}}',
    '.table-wrap{max-width:100%;overflow-x:hidden}.table-wrap table{width:100%;min-width:0;table-layout:fixed}.table-wrap th,.table-wrap td{min-width:0;padding-left:8px;padding-right:8px;white-space:normal;overflow-wrap:anywhere}.table-wrap th{font-size:10px;letter-spacing:.045em}.table-wrap th:first-child,.table-wrap td:first-child{width:38px}.table-wrap th:nth-child(5),.table-wrap td:nth-child(5){width:13%}.table-wrap th:nth-child(6),.table-wrap td:nth-child(6){width:9%}.table-wrap th:nth-child(11),.table-wrap td:nth-child(11){width:7%}.detail-row>td{width:auto!important;max-width:100%;overflow:hidden}.row-detail,.track-box,.track-layout,.track-plot,.track-svg{box-sizing:border-box;min-width:0;max-width:100%}.track-layout{width:100%;grid-template-columns:minmax(0,1fr) 210px}.track-plot{width:100%;overflow:hidden}.track-svg{display:block;width:100%;height:auto}@media(max-width:1200px){.data-row>td:nth-child(7),.data-row>td:nth-child(12),.data-row>td:nth-child(13),.data-row>td:nth-child(14),thead th:nth-child(7),thead th:nth-child(12),thead th:nth-child(13),thead th:nth-child(14){display:none}.table-wrap th:nth-child(5),.table-wrap td:nth-child(5){width:17%}.table-wrap th:nth-child(6),.table-wrap td:nth-child(6){width:12%}}@media(max-width:900px){.data-row>td:nth-child(2),.data-row>td:nth-child(8),.data-row>td:nth-child(10),thead th:nth-child(2),thead th:nth-child(8),thead th:nth-child(10){display:none}.row-detail{padding:12px}.track-box{padding:12px}.detail-metrics{grid-template-columns:repeat(auto-fit,minmax(105px,1fr))}.track-legend{grid-template-columns:1fr 1fr}.legend-regions{grid-column:1/-1}}@media(max-width:620px){.data-row>td:nth-child(9),thead th:nth-child(9){display:none}.table-wrap th,.table-wrap td{padding-left:5px;padding-right:5px;font-size:11px}.table-wrap th{font-size:9px}.track-legend{display:block}}',
    '@media(max-width:1050px){.track-layout{grid-template-columns:minmax(0,1fr)}.track-legend{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));width:100%}.legend-regions{grid-column:1/-1;display:flex;align-items:center;gap:12px;flex-wrap:wrap}.legend-regions>b{width:100%;margin:0}}',
    '.shell{width:calc(100% - 24px);max-width:1920px;padding-left:20px;padding-right:20px}',
    '.indicator-key{display:flex;align-items:flex-start;gap:22px;flex-wrap:wrap;margin-top:12px;padding-top:11px;border-top:1px solid #dddff2}.indicator-key-group{display:flex;align-items:center;gap:11px;flex-wrap:wrap}.indicator-key-group>strong{color:var(--ink);font-size:11px;text-transform:uppercase;letter-spacing:.06em}.indicator-key-item{display:inline-flex;align-items:center;gap:5px;white-space:nowrap;color:#52607e}.indicator-dot{display:inline-block;flex:0 0 auto;width:12px;height:12px;border:2px solid #ffffff;border-radius:50%;box-shadow:0 0 0 1px #26345c24}.data-row .indicator-dot{width:14px;height:14px}.table-wrap th:nth-child(2),.table-wrap td:nth-child(2),.table-wrap th:nth-child(3),.table-wrap td:nth-child(3){width:52px;text-align:center}.indicator-dot.awaiting-analysis{background:#9aa4ba}.indicator-dot.preliminary{background:var(--orange)}.indicator-dot.stabilized{background:var(--green)}.indicator-dot.stability-complete{background:#4e7ee8}.indicator-dot.type-ml{background:#e66f25}.indicator-dot.type-cpg{background:#18a9d1}.indicator-dot.type-both{background:#a340bb}@media(max-width:700px){.indicator-key{display:grid;grid-template-columns:1fr}.indicator-key-group{gap:8px}}',
    '.table-wrap th:nth-child(2),.table-wrap td:nth-child(2),.table-wrap th:nth-child(3),.table-wrap td:nth-child(3){padding-left:4px;padding-right:4px}.table-wrap th:nth-child(2)::after,.table-wrap th:nth-child(3)::after{display:none}',
    ...
    '<section class="panel"><div class="toolbar"><div><strong>Discovery table</strong> <span class="live" id="live">&#9679; loading snapshot</span></div><div class="controls"><select id="type-filter" aria-label="Filter discoveries by type"><option value="">All types</option><option value="ML centered">ML centered</option><option value="CpG centered">CpG centered</option><option value="CpG &amp; ML">CpG &amp; ML</option></select><select id="sort" aria-label="Order discoveries"><option value="combined">Best overall performance</option><option value="ml">Best ML R2</option><option value="cpg">Best CpG correlation</option><option value="column" hidden>Column sorting</option></select><input id="search" type="search" aria-label="Search discoveries" placeholder="Search gene, transcript, CpG or model"></div></div><div class="method-note"><strong>How ranking and discovery types are defined.</strong> Median R&sup2; measures held-out predictive performance of the complete ML model across seeds; CpG |&rho;| measures the strength of a monotonic association between the single best CpG and the target. They measure different properties and are shown together only as a reference&mdash;each should also be interpreted independently. <strong>ML centered</strong> means median R&sup2; is at least 0.01 higher than CpG |&rho;| on the ML dataset; <strong>CpG centered</strong> means the reverse; values within 0.01 are labeled <strong>CpG &amp; ML</strong>. Best overall performance orders by the larger available value, without averaging them or treating them as equivalent scales.<div class="indicator-key" aria-label="Table colour legend"><div class="indicator-key-group"><strong>Status</strong><span class="indicator-key-item"><i class="indicator-dot stabilized"></i>Stabilized</span><span class="indicator-key-item"><i class="indicator-dot stability-complete"></i>Stability complete</span><span class="indicator-key-item"><i class="indicator-dot preliminary"></i>Preliminary</span><span class="indicator-key-item"><i class="indicator-dot awaiting-analysis"></i>Awaiting analysis</span></div><div class="indicator-key-group"><strong>Type</strong><span class="indicator-key-item"><i class="indicator-dot type-ml"></i>ML centered</span><span class="indicator-key-item"><i class="indicator-dot type-cpg"></i>CpG centered</span><span class="indicator-key-item"><i class="indicator-dot type-both"></i>CpG &amp; ML</span></div></div></div><div class="table-wrap"><table><thead><tr>',
    '<th aria-label="Expand details"></th><th class="sortable" tabindex="0" data-sort-key="status">Status</th><th class="sortable" tabindex="0" data-sort-key="type">Type</th><th class="sortable" tabindex="0" data-sort-key="gene">Gene</th><th class="sortable" tabindex="0" data-sort-key="transcript">Transcripts</th><th class="sortable" tabindex="0" data-sort-key="best_cpg">Best CpG</th><th class="sortable" tabindex="0" data-sort-key="cpgs" data-sort-type="number">CpGs</th><th class="sortable" tabindex="0" data-sort-key="cpg_rho_original" data-sort-type="number">CpG |&rho;| original</th><th class="sortable" tabindex="0" data-sort-key="cpg_rho_ml" data-sort-type="number">CpG |&rho;| ML dataset</th><th class="sortable" tabindex="0" data-sort-key="model">Model</th><th class="sortable" tabindex="0" data-sort-key="median_r2" data-sort-type="number">Median R&sup2;</th><th class="sortable" tabindex="0" data-sort-key="seeds" data-sort-type="number">Seeds</th><th class="sortable" tabindex="0" data-sort-key="samples" data-sort-type="number">Samples</th><th class="sortable" tabindex="0" data-sort-key="resolved_by">Resolved by</th><th class="sortable" tabindex="0" data-sort-key="group">Group</th></tr></thead><tbody id="rows"></tbody></table><div class="empty" id="empty">Connect to a job to see its discoveries.</div></div><div class="note">Use the arrow button to expand a discovery; text in the other cells can be selected and copied normally. Original CpG |&rho;| comes from the initial per-CpG scan. ML-dataset CpG |&rho;| is recalculated for the same best CpG using the samples given to machine learning. Hover or focus a genomic-track point to see its signed original &rho;. Sample group identifies a metadata subset such as sex=F; All samples means no subset was applied. Use All types or a specific Type to filter discoveries. Click any column heading to sort it; click again to reverse the order. CpGs is the number of methylation probes evaluated in that computational group. A computational group may contain multiple biological transcripts only when their effective CpG and sample matrices are identical. Awaiting analysis means the group is known but model screening is not complete. Preliminary evidence comes from model screening. Stabilized evidence has completed the configured seed stability analysis.</div></section></main>',
    ...
    'function badge(v){const c=v.replace(/ /g,"-");return `<span class="indicator-dot ${c}" role="img" aria-label="Status: ${v}" title="Status: ${v}"></span>`}function typeBadge(v){let c=v.startsWith("ML")?"type-ml":v.includes("&")?"type-both":"type-cpg";return `<span class="indicator-dot ${c}" role="img" aria-label="Type: ${v}" title="Type: ${v}"></span>`}function resolverBadge(v){return v?`<span class="resolver">${esc(v)}</span>`:"\u2014"}',
...
ugPlotServer <- function(host = "0.0.0.0", port = 8080,
  ...
  pr$handle(
    "GET", "/reports/<job_id>/groups/<group_id>/track", function(job_id, group_id, res) {
      tryCatch({
        res$setHeader("Cache-Control", "no-store")
        ugplot_job_discovery_group_track(job_id, group_id, jobs_dir)
      }, error = function(e) {
        res$status <- 404
        list(error = conditionMessage(e))
      })
