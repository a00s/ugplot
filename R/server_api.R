...
ugplot_discovery_report_html <- function(job_id = "") {
  ...
  paste0('<!doctype html><html lang="en"><head><meta charset="utf-8">',
    ...
    'function esc(v){const d=document.createElement("div");d.textContent=v??"";return d.innerHTML}function render(){const q=$("search").value.toLowerCase();const mode=$("sort").value;const score=r=>{const ml=Number(r.median_r2),cpg=Math.abs(Number(r.cpg_rho));if(mode==="combined")return Number.isFinite(ml)&&Number.isFinite(cpg)?(ml+cpg)/2:Number.isFinite(cpg)?cpg/2:-Infinity;const v=mode==="cpg"?cpg:ml;return Number.isFinite(v)?v:-Infinity};const data=all.filter(r=>Object.values(r).join(" ").toLowerCase().includes(q)).sort((a,b)=>score(b)-score(a));$("rows").innerHTML=data.map(r=>`<tr><td>${badge(esc(r.status))}</td><td>${typeBadge(esc(r.type))}</td><td><b>${esc(r.gene)}</b></td><td>${esc(r.transcript)}</td><td>${esc(r.best_cpg)}</td><td>${fmt(r.cpg_rho)}</td><td>${esc(r.model)}</td><td><b>${fmt(r.median_r2)}</b></td><td>${fmt(r.min_r2)}</td><td>${fmt(r.max_r2)}</td><td>${fmt(r.seeds).replace(".000","")}</td><td>${fmt(r.samples).replace(".000","")}</td><td>${esc(r.group)}</td></tr>`).join("");$("empty").style.display=data.length?"none":"block"}$("search").oninput=render;$("sort").onchange=render;',
